...
def main():
	...
	copyToTempdir = r'xcopy "{source!s}" "{destination!s}" /E'.format(source = gameFolder, destination = tempDir)
	...
	subprocess.call(['svn', 'add', workingRepoDir + os.sep + name, '--force'])
	subprocess.call(['svn', 'commit', workingRepoDir + os.sep, '-m', commit_message, '--username', repoUser, '--password', repoPassword])
