...
def pootDiff(wiki, patchName, diffdata):
	...
	for r in fileRe.finditer(diffdata):
		...
		dupe_check_list = [x["name"] for x in files]
		
		for q in dupe_check_list:
			if f == q:
				if contents.find("+application/octet-stream") != -1:
					# If this is the 'added' entry, remove the previous entry
					files.remove(files[dupe_check_list.index(q)])
				else:
					append = False
	...
	def formatFile(f):
		...
		for l in lines:
			...
			if lineRes is not None:
				...
				isAdd = True
			elif l[:14] == u'Cannot display' or l.find("+application/octet-stream") != -1:
				isBinary = True
